...
class CartController implements Controller
{
  ...
  public function initPage()
  {
    ...
    if ($userPreference && $userPreference[0]) {
      $this->popupModel->setPopupInfo('confirm', 'Předešlá objednávka', 'V minulosti jste si objednával. Chcete obnovit minulou objednávku?');
    }
  ...
  public function removeItem()
  {
    ...
    if ($index === null || !$removeItem) $this->popupModel->setPopupInfo('error', 'Chyba', 'Položku nelze z košíku vymazat. Zkuste to znovu.');
  ...
  public function removeAllItems()
  {
    ...
    if (!$removeAll) $this->popupModel->setPopupInfo('error', 'Chyba', 'Košík je prázdný. Nelze odebírat položky.');
    $this->finishOrderModel->setFormData($_POST);
    header('Location: ../cart.php');
  }
  ...
  public function buyOrder()
  {
    ...
    if (empty($cart)) {
      $this->popupModel->setPopupInfo('error', 'Chyba', 'Košík je prázdný. Objednávku nelze provést.');
      $this->finishOrderModel->setFormData($_POST);
      header('Location:../cart.php');
      exit;
    } else {
